    ...
    <div class="container">
      ...
      <section>
        ...
        <div class="button-group">
          ...
          <button class="btn btn-accent" @click="show({ title: 'Файлы', body: AppFileUpload })">Загрузка</button>
        </div>
      </section>
    </div>
  </div>
      ...
      setup() {
        ...
        return () => h('div', { style: 'display: flex; flex-direction: column; gap: 20px;' }, [
          ...
          h('div', { style: 'text-align: center;' }, [
            h('p', { style: 'font-weight: 600; margin-bottom: 12px; text-align: left' }, 'Ваш будущий напарник:'),
            h('img', { 
...
const showQueue = (num: number) => {
  show({
    ...
    footer: defineComponent({
      setup: () => () => h('div', { style: 'display: flex; gap: 8px' }, [
...
const showLogin = () => {
  show({
    ...
    footer: defineComponent({
      setup: () => () => h(AppButton, { label: 'Войти', style: 'width: 100%', onClick: close })
    })
  });
};
